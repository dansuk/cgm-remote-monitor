...
daytoday.report = function report_daytoday(datastorage,sorteddaystoshow,options) {
  ...
  d3.select('body').append('svg')
    ...
    var morningLimits = {
      start: (new Date(day)).setHours(7),
      end:(new Date(day)).setHours(12)
    };
    var afternoonLimits = {
      start: (new Date(day)).setHours(12),
      end:(new Date(day)).setHours(18)
    };
    var dinnerLimits = {
      start: (new Date(day)).setHours(18),
      end:(new Date(day)).setHours(24)
    };

    var timeLimits = [morningLimits, afternoonLimits, dinnerLimits];
    var carbratioAM = { totCarb:0, totBolus:0, time:morningLimits};
    var carbratioFM = { totCarb:0, totBolus:0, time:afternoonLimits};
    var carbratioDinner = { totCarb:0, totBolus:0, time:dinnerLimits};
    var carbRatios =[carbratioAM, carbratioFM, carbratioDinner];
...
//            .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (t.absolute ? Number(t.absolute).toFixed(2) + 'U' : ''));
    ...
    if (options.insulindistribution) {
      ...
      $('<tr><td><b>' + 'C ratio' + toTimeString(new Date(carbRatios[0].time.start)) + ' to ' + toTimeString(new Date(carbRatios[0].time.end)) + '</b></td><td align="right">' + (carbRatios[0].totCarb/carbRatios[0].totBolus).toFixed(1) + 'g/U</td></tr>').appendTo(table);
      $('<tr><td><b>' + 'C ratio' + toTimeString(new Date(carbRatios[1].time.start)) + ' to ' + toTimeString(new Date(carbRatios[1].time.end)) + '</b></td><td align="right">' + (carbRatios[1].totCarb/carbRatios[1].totBolus).toFixed(1) + 'g/U</td></tr>').appendTo(table);
      $('<tr><td><b>' + 'C ratio' + toTimeString(new Date(carbRatios[2].time.start)) + ' to ' + toTimeString(new Date(carbRatios[2].time.end)) + '</b></td><td align="right">' + (carbRatios[2].totCarb/carbRatios[2].totBolus).toFixed(1) + 'g/U</td></tr>').appendTo(table);
      console.log("carbRatio " + day+ "carb " + carbRatios[2].totCarb + " tot inulin " +carbRatios[2].totBolus);
      console.log("hej");
    ...
    function toTimeString(date){
        return minTwoDigits(date.getHours())+':'+ minTwoDigits(date.getMinutes());
    }
